    ...
    switch(type){
      ...
      case 'combo':
        ...
        for(let i=0;i<animationREFS.length;i++){
          animationREF = animationREFS[i];
          if (animationREF.nodeName == "SPANREF"){
            var animationREFID = this.reader.getString(animationREF, 'id');

            if (this.animations[animationREFID] == null )
                return "ID must exist";
            else if(this.animations[animationREFID] instanceof ComboAnimation){
              return "ID belongs to another ComboAnimation";
            }
            else{
              animations.push(this.animations[animationREFID].clone());
            }
          }
        }
...
MySceneGraph.prototype.parseAnimationsCPoints = function(animationNode){
  ...
  for(i=0;i<cplines.length;i++){
      cpoint = cplines[i];
      if (cpoint.nodeName == "controlpoint"){
        ...
        if ((nodeName = children[i].nodeName) == "ROOT") {
            ...
            if(animationsIndex != -1 ){
              ...
              for (let j = 0; j < animations.length; j++) {
                if (animations[j].nodeName == "ANIMATIONREF"){
                  ...
                  this.nodes[nodeID].addAnimation(this.animations[animationID]);
                }
              }
            }
            ...
            for (var j = 0; j < descendants.length; j++) {
                if (descendants[j].nodeName == "NODEREF")
				{
					...
					{
						var type=this.reader.getItem(descendants[j], 'type', ['rectangle', 'cylinder', 'sphere', 'triangle', 'patch', 'board', 'button']);
